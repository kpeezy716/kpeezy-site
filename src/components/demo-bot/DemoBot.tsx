    ...
    <div className="phone">
      <div className="island" /><div className="phone-screen">
        ...
        <div className="chat" ref={chat}>
          ...
          {!state.completed && currentStep?.options && state.messages.length > 0 && (plan.id === "miniapp" ? <MiniApp currentStep={currentStep.botMessage} options={currentStep.options} miniApp={currentStep.miniApp} disabled={state.isTyping} onChoose={choose}/> : <div className="bot-options">{currentStep.options.map((option) => <button disabled={state.isTyping} onClick={() => choose(option)} key={option.id}>{option.label}</button>)}</div>)}
...
function MiniApp({ currentStep, options, miniApp, disabled, onChoose }: { currentStep: string; options: NonNullable<DemoScenario["steps"][string]["options"]>; miniApp?: DemoScenario["steps"][string]["miniApp"]; disabled: boolean; onChoose: (option: NonNullable<DemoScenario["steps"][string]["options"]>[number]) => void }) { return <div className="mini-app"><div className="mini-app-head"><span>{miniApp?.title ?? "Мини-приложение"}</span><b>×</b></div>{miniApp && <div className="mini-app-tabs" aria-label="Разделы мини-приложения">{miniApp.tabs.map((tab) => <span key={tab}>{tab}</span>)}</div>}<h4>{miniApp?.note ?? currentStep}</h4><div className="mini-app-cards">{options.map((option, index) => <button disabled={disabled} onClick={() => onChoose(option)} key={option.id}><i>{index % 2 === 0 ? <CalendarDays size={18}/> : <ClipboardList size={18}/>}</i><span>{option.label}</span><ChevronRight size={16}/></button>)}</div></div>; }
